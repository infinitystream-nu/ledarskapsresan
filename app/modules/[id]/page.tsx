...
export default function ModulePage({ params }: Props) {
  ...
  const tabs = mod.videoUrl
    ? ["video", "fordupning", "ovningar"]
    : ["fordupning", "ovningar"];

  function tabLabel(tab: string) {
    if (tab === "video") return "Presentation";
    if (tab === "fordupning") return "Fördjupning";
    return "Övningar";
  }
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex gap-1 mb-6 border-b border-gray-200">
            {tabs.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab as "video" | "fordupning" | "ovningar")}
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="aspect-video">
                {isDev ? (
                  ...
                  <iframe src={mod.videoUrl} className="w-full h-full" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen />
                )}
              </div>
              ...
              {ovningar?.questions && ovningar.questions.length > 0 ? (
                <>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-5">
                    ...
                    <Link href={`/coach?moduleId=${mod.id}`} className="inline-block text-sm bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
                      Öppna AI-coachen →
                    </Link>
                  </div>
                </>
